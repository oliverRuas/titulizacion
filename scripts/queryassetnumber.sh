...
# x=echo $NUMERO
echo peer chaincode invoke -o $ORDERER_ADDRESS --tls true --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CC_NAME -c $NUMERO --peerAddresses $CORE_PEER_ADDRESS --peerAddresses localhost:8051 --peerAddresses localhost:10051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE --tlsRootCertFiles  $(cd .. && echo $PWD/fabric-ca/farmer/peers/farmer-peer/tls/ca.crt) --tlsRootCertFiles  $(cd .. && echo $PWD/fabric-ca/spv/peers/spv-peer/tls/ca.crt) > transfer_tokens.sh
